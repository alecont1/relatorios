...
must_haves:
  truths:
    ...
    - "User sees tenant brand colors applied throughout the application UI"
  artifacts:
...
2. Update frontend/src/App.tsx with these specific changes:
...
**B. In App component, add useTheme() call at the start (before return):**
```tsx
function App() {
...
**C. In AppLayout component's navigation section (after existing Usuarios link), add:**
```tsx
{/* After Usuarios link */}
...
**D. In Routes section (before the catch-all /* route), add:**
```tsx
...
Summary of App.tsx modifications:
- 3 new imports (useTheme, TenantsPage/TenantSettingsPage, Building2/Settings icons)
- useTheme() call in App function body
- 2 new navigation links in AppLayout (conditional on role)
- 2 new Route elements with ProtectedRoute wrappers
  </action>
